...
const ThemeSwitch = () => {
  ...
  return (
    <div className="relative inline-block w-[69.667px] cursor-pointer">
      <label htmlFor="theme-toggle" className="block">
        ...
        <svg viewBox="0 0 69.667 44" xmlns="http://www.w3.org/2000/svg" className="w-full">
          <g transform="translate(3.5 3.5)" id="Component_15_1">
            ...
            {/* Toggle button */}
            <g 
              transform={`translate(${isDark ? '30.333' : '2.333'} 2.333)`} 
              id="button"
              className="transition-all duration-300 ease-in-out"
            >
              {/* Sun */}
              <g id="sun" className={`transition-opacity duration-300 ${isDark ? 'opacity-0' : 'opacity-100'}`}>
                <circle fill="#f8e664" r="15.167" cy="15.167" cx="15.167" id="sun-outer-2" />
                <path fill="rgba(246,254,247,0.29)" d="M11.667,0A11.667,11.667,0,1,1,0,11.667,11.667,11.667,0,0,1,11.667,0Z" transform="translate(3.5 3.5)" id="sun-3" />
                <circle fill="#fcf4b9" transform="translate(8.167 8.167)" r={7} cy={7} cx={7} id="sun-inner" />
              </g>
              
              {/* Moon */}
              <g id="moon" className={`transition-opacity duration-300 ${isDark ? 'opacity-100' : 'opacity-0'}`}>
                <circle fill="#cce6ee" r="15.167" cy="15.167" cx="15.167" id="moon-3" />
            ...
            <g className={`transition-opacity duration-300 ${isDark ? 'opacity-0' : 'opacity-100'}`}>
              <path fill="#fff" d="M3512.81,173.815a4.463,4.463,0,0,1,2.243.62.95.95,0,0,1,.72-1.281,4.852,4.852,0,0,1,2.623.519c.034.02-.5-1.968.281-2.716a2.117,2.117,0,0,1,2.829-.274,1.821,1.821,0,0,1,.854,1.858c.063.037,2.594-.049,3.285,1.273s-.865,2.544-.807,2.626a12.192,12.192,0,0,1,2.278.892c.553.448,1.106,1.992-1.62,2.927a7.742,7.742,0,0,1-3.762-.3c-1.28-.49-1.181-2.65-1.137-2.624s-1.417,2.2-2.623,2.2a4.172,4.172,0,0,1-2.394-1.206,3.825,3.825,0,0,1-2.771.774c-3.429-.46-2.333-3.267-2.2-3.55A3.721,3.721,0,0,1,3512.81,173.815Z" transform="translate(-3466.47 -160.94)" id="cloud" />
            </g>
            
            {/* Stars */}
            <g fill="#def8ff" transform="translate(3.585 1.325)" id="stars" className={`transition-opacity duration-300 ${isDark ? 'opacity-100' : 'opacity-0'}`}>
